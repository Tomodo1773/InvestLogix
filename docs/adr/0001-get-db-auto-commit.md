...
- 将来ユーザー規模が拡大し PgBouncer の transaction mode 等への移行を検討する際にも、「リクエスト境界 = トランザクション境界」が揃っていることは前提条件となる。今すぐ移行する必要はないが、この整理によって退路を塞がずに済む（※移行時は別途、RLS 用セッション変数のトランザクションスコープ化や asyncpg の prepared statement 無効化が必要）
