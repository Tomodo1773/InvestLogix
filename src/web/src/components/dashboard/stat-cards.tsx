...
export function StatCards({ summary, isLoading }: StatCardsProps) {
  const stats = [
    ...
    },
    {
      ...
      icon: TrendingUp,
      color: summary && Number(summary.total_pl_percentage) >= 0 ? "text-[#4CAF50]" : "text-destructive",
      bgColor: summary && Number(summary.total_pl_percentage) >= 0 ? "bg-[#4CAF50]/10" : "bg-destructive/10",
    },
    {
      label: "Market Value",
