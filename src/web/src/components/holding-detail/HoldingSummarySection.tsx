            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-5">
              ...
              <div className="space-y-1">
                ...
                <div className="text-lg font-semibold">
                  <p>{holding.market_value !== null ? formatCurrency(holding.market_value, "JPY") : "-"}</p>
                  {holding.currency === "USD" ? (
                    <p className="text-sm text-muted-foreground">
                      {holding.market_value_usd !== null
              ...
              <div className="space-y-1">
                ...
                <p className={`text-lg font-semibold ${getPLColorClass(holding.total_pl)}`}>
                  {holding.total_pl !== null ? formatCurrency(holding.total_pl) : "-"}
                </p>
              </div>
              <div className="space-y-1">
                ...
                <p className={`text-lg font-semibold ${getPLColorClass(holding.total_pl_percentage)}`}>
                  {holding.total_pl_percentage !== null ? formatPercent(holding.total_pl_percentage) : "-"}
                </p>
              </div>
            </div>

            <div className="space-y-2 border-t pt-4">
              <p className="text-sm text-muted-foreground">口座別保有数</p>
              {holding.account_holdings.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {holding.account_holdings.map((account) => (
                    <div
                      key={account.account_type}
                      className="flex items-baseline gap-2 rounded-md border bg-muted/30 px-3 py-2"
                    >
                      <span className="text-sm text-muted-foreground">{account.account_type}</span>
                      <span className="font-semibold">{account.quantity.toLocaleString()}株</span>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-sm text-muted-foreground">口座別の保有情報がありません</p>
              )}
            </div>
          </div>
        ) : (
